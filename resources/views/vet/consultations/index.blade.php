                ...
                <div class="p-6">
                    <div class="flex flex-col md:flex-row md:items-center md:justify-between">
                        <div class="flex items-center mb-4 md:mb-0">
                            <img src="{{ $consultation->user->profile_image }}" alt="{{ $consultation->user->name }}" class="w-12 h-12 rounded-full object-cover mr-4">
                            <div>
                                <h3 class="text-lg font-semibold text-gray-800">{{ $consultation->user->name }}</h3>
                                <p class="text-gray-600">{{ $consultation->animal_type }} - {{ $consultation->animal_name }}</p>
                            </div>
                        </div>
                        <div class="flex flex-col items-end">
                            <div class="flex items-center mb-2">
                                ...
                                <span class="text-gray-700">{{ \Carbon\Carbon::parse($consultation->created_at)->format('d M Y') }}</span>
                            </div>
                            <div class="flex items-center">
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-gray-500 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
                                </svg>
                                <span class="text-gray-700">{{ $consultation->status === 'completed' ? $consultation->duration . ' min' : 'En attente' }}</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="mt-4 pt-4 border-t border-gray-100">
                        <div class="flex flex-col md:flex-row md:items-center md:justify-between">
                            <div>
                                ...
                                <p class="text-gray-600 mt-1">{{ Str::limit($consultation->description, 100) }}</p>
                                <div class="mt-2">
                                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {{ 
                                        $consultation->status === 'pending' ? 'bg-yellow-100 text-yellow-800' : 'bg-green-100 text-green-800' 
                                    }}">
                                        {{ $consultation->status === 'pending' ? 'En attente' : 'Terminée' }}
                                    </span>
                            ...
                            <div class="mt-4 md:mt-0">
                                @if($consultation->status === 'pending')
                                    <a href="{{ route('vet.consultations.respond', $consultation->id) }}" class="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-green-600 hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500">
                                        Répondre
                                    </a>
                                @else
                                    <a href="{{ route('vet.consultations.show', $consultation->id) }}" class="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-green-600 hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500">
                                        Voir détails
                                    </a>
                                @endif
                            </div>
